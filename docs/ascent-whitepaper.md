...
- **Function bodies are just blocks — no arrow.** A function body is a `{ … }` block exactly like an `if` / `for` / `program` block, and by the block-value rule (§2) it yields the value of its last statement — so `fn(x: Int) -> Int { x + 1 }` returns `x + 1` with no `return` needed. There is **one** body form: the block. An arrow (`=>`) form was considered and removed, because it saved nothing (`{ e }` is the same two characters as `=> e`) and added a function-specific second spelling on top of the block-value rule a learner already knows from `if (c) { value }` — the very pattern `fn(...) { value }` reuses. Dropping it also removes the awkward `-> T =>` double-arrow in annotated functions, and leaves `->` meaning just one thing (a return type, and a `match` arm). `return` remains early-exit only (§5).
