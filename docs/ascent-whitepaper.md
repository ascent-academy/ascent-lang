...
- **Discarding a value is explicit — `void` — and the rule is about *position*.** The unifying principle: **a value must go somewhere — consumed, or explicitly discarded, never silently dropped.** Some positions are **`Done`-required**: the block sitting there is *expected to yield `Done`*, because nothing consumes its value. **A `Done`-required block may not end in a non-`Done` expression** — if it does, that trailing value must be explicitly discarded with **`void`** (`void expr`), or it is a compile error. `void expr` evaluates `expr` and throws its result away — exactly TypeScript's `void expr`, so it transfers as the same "intentionally ignore this" marker rather than a false friend.

  A position is `Done`-required exactly when **nothing consumes the block's value**, which arises in two ways:
  - **A non-final statement** of a block — another statement follows, so this one's value is discarded. In `foo(); bar()`, if `foo()` returns non-`Done` you write `void foo();`.
  - **A loop body** (`for` / `while`) — the loop as a whole yields `Done` (§5), so it *ignores* whatever its body block yields on each iteration. `for x in xs { x + 1 }` is an error (the body ends in a non-`Done` `Int` the loop discards); write `void x + 1`, or — if you meant to *collect* the results — use `.map`, which is what value-producing iteration actually is. (The error doubles as the teaching moment that a `for` loop does not build a value.)

  Everything else is a **value position**, where the block's final expression *is* its value and flows out normally — so these are unconstrained: the last statement of a block, a function body, the `program` body, and a *value*-position `if` / `match`. And nothing needs `void` where there is nothing to drop: a `Done`-valued statement (an effectful `print`, a loop) already yields `Done`; a value that is bound (`fix` / `mut`), passed to a function, or used inside an expression is consumed. So the rule fires *only* on a bare non-`Done` value in a `Done`-required position — exactly the silent-no-op bug.

  Two consequences fall out of the same rule rather than being separate cases: a statement-position **`if` / `match` used purely for effect** sits in a `Done`-required position, so if its branches yield a non-`Done` value the whole expression takes the `void` (`void if (c) { a() } else { b() }`); and Ascent has **no `void` *type*** — a function returning nothing returns `Done` (§4), so `void` is exclusively this discard keyword and a C/Java arrival should not expect it as a return type.
...
- **`match subject { }`** — an expression, **exhaustiveness-checked**. v1 patterns are **shallow**, and there are three kinds: **literal** (`0`, `"hi"`, `True`), **variant** (`Circle{ r }` — matches a union case and binds its fields), and **catch-all** (the default arm, in two forms — see below). A variant pattern may bind a **subset** of the variant's fields — the ones you name are bound, the rest are simply ignored (`Circle{ r }` binds `r`, ignores any other field) — so there is no separate field-wildcard to learn. (No nested patterns, guards, or or-patterns in v1.) Chosen over `switch` to avoid fallthrough/`break` expectations.
  - **Catch-all has two spellings — binding and non-binding — and they are the same mechanism** ("match everything not yet matched"), differing only in whether the value is kept: **`else`** catches the rest and binds nothing (`else -> "many"` — reusing the `else` from `if` / `try … else`, so "0, 1, or **else** many" reads with no explanation, and there is no cryptic `_`); a **bare identifier** catches the rest and **binds it** to that name (`value -> "hello ${value}"`). You pick the spelling by whether you need the value. This bare-identifier form is how an **`Optional` is destructured** — since `T?` is `None | value` with no `Some` wrapper (§9), the present case has no constructor to name, so you bind it directly:

  ```ascent
  match name {
      None  -> "no name given",
      value -> "hello, ${value}",   # `value` is the present String — narrowed to T (§7)
  }
  ```

  - **At most one catch-all, and it must be last.** Because both spellings mean "the rest," a `match` has *zero or one* catch-all arm — an `else` **or** a bound identifier, never both (a second catch-all is unreachable, since the first already consumed every remaining case). For the same first-match-wins reason (§12) it must be the **final** arm; any arm after a catch-all is an **unreachable-arm error**.
  - **Exhaustiveness + catch-all:** a `match` must cover every case — list all variants, *or* supply a catch-all (`else` or a binding identifier). A catch-all makes a `match` trivially exhaustive, which is convenient but **masks future variants**: add a `Blue` to a `Color` matched with `Red` / `Green` / `else`, and the catch-all silently swallows it instead of the checker flagging the gap — so prefer listing variants explicitly when you want the compiler to catch later additions, and use a catch-all only when the remaining cases genuinely do not matter.
  - **A catch-all after full coverage is an unreachable-arm error.** The two ways to be exhaustive are mutually exclusive: once every variant is *already* listed, a catch-all can match nothing — the **residual** (the values not caught above) is empty, i.e. **`Never`** — so it is dead code and rejected. For the binding form the tell is direct: the bound name would have type `Never`, a value that cannot exist. This is the *same* rule as "at most one catch-all" above — both are just *an arm whose residual is empty is unreachable*, detected by the residual narrowing to `Never` (a second catch-all empties the residual; so does a complete variant list). So a `match` is either *all variants listed, no catch-all* **or** *some variants plus one catch-all* — never all-variants-plus-catch-all. Happy consequence: because a fully-listed `match` is *forbidden* from carrying a masking catch-all, adding a variant later necessarily re-triggers the exhaustiveness error — so the future-variant safety the previous point asks for is **automatic** whenever you list completely, not merely advised. (This bites only for **finite** types — unions and `Bool`; `Int` / `String` literals can never exhaust the value space, so their residual is never `Never` and a catch-all there is always legal and needed.)
- **Irrefutable patterns in a binding — one-line destructuring, the honest replacement for tuples.** A `fix` / `mut` binding may hold a *pattern* instead of a plain name, when that pattern is **irrefutable** (cannot fail to match) — which, for Ascent's types, means a **single-variant record**. This destructures a multi-field value into named locals in one statement:
  ...
  This is the good half of tuples — one-statement multi-value binding — *without* the bad half: the fields bind **by name** (`quotient`, `remainder`), not by position (`.0` / `.1`), so binding order is irrelevant and a swap is impossible (`fix DivResult{ remainder, quotient } = …` still binds each correctly) — strictly safer than positional tuple destructuring, which silently swaps same-typed values. It reuses the exact pattern syntax of `match` arms (field punning binds locals named for the fields; `DivResult{ quotient: q }` renames), just in binding position. **Refutable** patterns — a *union* variant (`Red`, `Success{ … }`) that might not match — are **not** allowed in a `fix` / `mut` binding; they require `match`, so the other cases must be handled (the Rust / ML `let Point{ x, y } = p` rule). This is why Ascent needs no tuples: a multi-value return is a *named* record (self-documenting, no `.0` / `.1`), and destructuring it is this one-liner.
- **`while (cond) { }`** for condition loops. **`for x in xs`** iterates values and takes **no** parens — it has no test, and parenthesizing it would mimic TypeScript's *key*-iterating `for…in`, the very false friend the `in`-for-values choice avoids. No C-style three-part `for`. **Both are statements that yield `Done`** — a loop has no single meaningful result (an empty collection, or a `while` that runs zero times, has no last value to give), while a provably-infinite `while (True)` is typed `Never` (§7). Producing a value *from* a sequence is the collection API's job — `reduce`, `find` (→ `T?`), `map`, `filter` — never loop-return; this keeps the block-value rule (§2) special-case-free — a loop body is a **`Done`-required position** (§2), so an effect body (`print(x)`) already yields `Done`, while a body ending in a non-`Done` value must discard it (`for x in xs { void x + 1 }`) or use `.map` to collect.
...
- **Operator precedence**, loosest to tightest: `or` · `and` · `not` · comparisons (`== != < <= > >=`, non-associative — no chaining) · `??` (Optional-coalescing, **right-associative**) · `+ -` · `* / div mod` · unary `-` · `**` (right-associative, tighter than unary minus) · atoms (literals, identifiers, parenthesized expressions). Binary arithmetic is left-associative. Follows Python in one respect: `not` binds looser than comparison, so `not a == b` parses as `not (a == b)`. `??` follows Swift's nil-coalescing placement — tighter than comparison, looser than arithmetic — so `xs.at(i) ?? 0 == 0` groups as `(xs.at(i) ?? 0) == 0`, and being right-associative, `a ?? b ?? c` is `a ?? (b ?? c)` (a chain of fallbacks). The expression parser is Pratt-style (§12).
- **Function bodies are just blocks — no arrow; return type after a colon.** A function body is a `{ … }` block exactly like an `if` / `for` / `program` block, and by the block-value rule (§2) it yields the value of its last statement — so `fn(x: Int): Int { x + 1 }` returns `x + 1` with no `return` needed. The **return type follows a colon** — `fn(params): Ret { body }` — matching the parameter annotations (`x: Int`) and TypeScript's function-declaration form, and a character shorter than an arrow. There is **one** body form: the block. (A single-expression arrow form, `=>`, was considered and removed: it saved nothing — `{ e }` is the same two characters as `=> e` — and layered a function-specific second spelling on the block-value rule a learner already knows from `if (c) { value }`, the very pattern `fn(...) { value }` reuses.) `return` remains early-exit only (§5).
- **A function *type* is written `Fn(Params) -> Ret` — arrow, and capitalized.** The colon is the *declaration* return; a function *type expression* (the type of a function value) keeps an **arrow**: `Fn(Int) -> String`, `Fn(T) -> U`. Capital `Fn` because it is a *type* (UpperCamel, like `List` / `Optional`), while lowercase `fn` is the *value* (a function literal, like `[1, 2, 3]`). This split is deliberate — and it is exactly TypeScript's (declarations use `:`, function types use an arrow) — because the colon becomes hard to read the moment function types **nest**: a higher-order signature like `map(f: Fn(T) -> U): List<U>` stays legible with the arrow marking "`T` maps to `U`," whereas `map(f: Fn(T): U): List<U>` stacks three colons the reader must disentangle. So the common case (a plain declaration) gets the short colon, and the rare case (a function type, seen mostly in stdlib signatures a beginner *reads*) gets the arrow that stays clear when nested. `->` thus keeps one meaning across the language — "produces / maps to," as a `match`-arm result and as a function-type return — while the declaration colon is the ordinary annotation colon (`x: Int`, `field: Type`, and now the return) doing its usual "this has this type" job.
...
- **The single-variant / record form — and why you don't write the name twice.** A type with one variant is a **record**: a fixed set of named, typed **fields** (`User` above is a record with fields `name` and `age`). Its full form names the sole constructor explicitly — `type User = User{ name: String, age: Int }` — but because that constructor conventionally shares the type's name, the **`type X = { fields }`** form is sugar that **auto-names the constructor after the type**: `type User = { name: String, age: Int }` *is* `type User = User{ name: String, age: Int }`. So you never write `User` twice unless you want to; the bare-brace form is the everyday spelling. (This is the fielded version of the `Done` pattern — a single-variant type whose constructor shares its name, §2.) You still build and match with the constructor name (`User{ name: "Ann", age: 30 }`), and a multi-variant type never elides — its variants have distinct names (`Circle`, `Rect`), so there is nothing to repeat. **Terminology:** a single-variant type is a *record*, the `{ … }` holds its *fields*; not an "object" (which would wrongly import identity, mutation, and attached methods — none apply to these immutable, structurally-compared values) nor a "struct" (which carries a memory-layout connotation and is an abbreviation) — "record" is accurate and transfers to Elm / F# / Haskell.
- **Bare `{ … }` is legal *only* at the type-declaration head.** The name-eliding brace form exists solely as `type X = { … }`; a bare `{ … }` in any other position — a field's type, a parameter, a return type — is **not** a value or an anonymous record type (§6 forbids those, §7 stays structural-free). Everywhere else you name the type: `fn(p: User)`, never `fn(p: { name: String })`. The sugar auto-supplies the *constructor* name at a declaration; it never introduces a floating structural type.
- **Field access rule:** `e.field` is legal **iff `e`'s type has exactly one variant** (a record). Multi-variant types must be inspected with `match`. (The error message itself teaches sum types.)
...
- **Guarded construction — the `make { }` section makes invalid states unrepresentable.** A type may carry a **`make { }` section** (a sibling of `methods { }`) holding one or more **named validating constructors**, each typically returning `T orfail E`:

  ```ascent
  type HexColor = HexColor{ value: String }
    make {
        fromString: fn(s: String): HexColor orfail HexError {
            # validate "#RRGGBB"...
            HexColor{ value: s }        # raw constructor — reachable ONLY inside make / methods
        },
        fromRgb: fn(r: Int, g: Int, b: Int): HexColor orfail HexError {
...
- **Update with `with` — braceless for a single change, `{ }` for several, one form for every shape.** A new value derived from an existing one, with some positions replaced, is written `base with path = value` — no braces for a single update — or `base with { p1 = v1, p2 = v2, ... }` when there are several. A **path** is a chain of steps — a `.field` step or an `[index]` step — freely mixed: it is exactly the *access* expression you would write to read that position, minus its root. So the update mirrors the read:

  ```ascent
  user  with name = "new"                        # single field — braceless
  xs    with [3] = 42                            # single index — braceless
  grid  with [2][5] = 99                         # single 2-D index — braceless
  model with users[3].address.city = "Prague"    # single deep path — braceless
  model with count = its.count + 1               # `its` is the base — navigate to its fields
  order with { total = its.total * 1.2, paid = True }   # several updates → braces
  ```
  ...
  - **Braces only for multiple updates.** A single field/path update is braceless (`base with path = value`); the moment there are several, comma-separated, they are wrapped in `{ }`. (Braces are also allowed for a single update if you prefer.)
  ...
  - **`its`** is the **base value being updated**, referred to by its third-person possessive and navigated exactly like a read: `its.count`, `its.width * its.height`, `its.items.append(x)`. So a value derived from itself is explicit (`count = its.count + 1` reads "the base's count, plus one"), and — unlike a per-path pronoun — you can reference *other* fields of the base too (`area = its.width * its.height`). `its` is a contextual keyword, meaningful only on the right of a `with` update. (It replaces an earlier per-path `it`, a vaguer pronoun; `its.field` says exactly what it reaches, and reuses the ordinary read-navigation grammar rather than introducing new magic.)
  ...
  - The braced form's `{ … }` is **not** an anonymous record (those do not exist — above); it is update syntax bound to the base's type, whose fields and paths are checked against that type, so typos and wrong paths are caught.

  This **replaces the old `{ ...base, field }` spread**: since records are nominal, the spread's only real job was single-base update, which `with` now does across every shape — single field, deep field, index, 2-D, mixed — with assignment notation that transfers. There is no separate list-vs-record update form; the same `with` covers both, the path's steps saying which.
...
- **Intrinsic traits — the bound half of the trait system, compiler-known and consumed-not-defined.** Several built-in operations require a *capability* of a type: string interpolation's `${}` hole needs **`Display`** (§4), `sort` / `min` / `max` need **`Comparable`** (§15), and `for x in xs` needs **`Iterable`**. These capabilities are **traits** — but a trait is a **bound on a type parameter, not a type**. So a trait-constrained operation is *generic*, with the trait as a constraint on its parameter, never a parameter *of* the trait's type:

  ```ascent
  fix show = fn<T: Display>(x: T): String { ... }   # T is the type; Display is a bound on T
  ```

  Writing `fn(x: Display)` — trait-as-type — is **wrong** and unsupported: it would mean a boxed, type-erased value dispatched dynamically at runtime (a "trait object"), which drags in the runtime type interrogation and dynamic dispatch the nominal system excludes (§6). Ascent's traits are **static only** — resolved at compile time per concrete `T`, monomorphized, no trait objects in v1. *Intrinsic* traits are the **bound-checking half** of that system with **no user-facing declaration**: the compiler knows `Display` / `Comparable` / `Iterable` as built-in bounds, satisfied by a **compiler-fixed set of implementors** (the built-in scalars for `Display` / `Comparable` today — the same "hard-coded until traits exist" state §4 and §15 describe). Users *consume* these bounds (interpolate, `sort`, `for`) but cannot yet *define* a trait or implement one — exactly the "consumable, not definable" line above. (`Display` is the simplest — just "has a canonical string form." `Iterable` is the one that forces **associated types** — `for x in xs` must know the *element* type, `xs`'s `Item` — so build `Display` first, `Iterable` second; see `traits-open-questions.md`.)
- **Generic / bound syntax — `fn<T: Bound>(params): Ret`, angle brackets, not `where`.** Type parameters are declared in angle brackets **on `fn`, before the value parameters**: `fn<T>(...)` for an unbounded parameter, `fn<T: Display>(...)` with a bound, `fn<T, U>(...)` for several, `fn<T: Comparable + Display>(...)` for multiple bounds on one (the `+` combiner). Angle brackets are chosen over a trailing `where` clause because the constraint is then stated **before `T` is first used** (declaration-before-use, read top-down) and does not visually collide with the `: Ret` return type, where a `where` clause would sit awkwardly. This is the *declaration* counterpart of the `<…>` you already write at a *use* site (`List<Int>`) — same brackets, two roles (declare on `fn`, apply on a type). **Beginner-invisible:** because generics are consumable-not-definable, this form appears only in *stdlib signatures* — a learner writes bracket-free `fn(x: Int): Int`, and meets `fn<T: Display>(…)` only by inspecting a library function's type, never in their own code, until user-defined generics arrive (§16).
...
```ascent
fix userTask = fetchUser!(id);   # Task<User> — arguments bound, body NOT run, nothing happening yet
```
...
2. **Propagate failure** — a child's failure cancels its siblings, then re-raises in the parent as an ordinary `orfail` / `Result` (§9). The nursery is the *join point for failures*, the concurrent twin of errors propagating up a call stack.
...
- `all` — collect results; on the first `Failure`, cancel the rest and propagate (fail-fast).
- `gather` — wrap each task so its error becomes a *value* (`Result`); now nothing "fails," so the nursery cancels nothing and every outcome is collected (Success and Failure side by side).
...
**Staging.** A beginner's first async is a single `await fetchUser!(id)` — one inert task, started and awaited on the spot; *no* concurrency, because they do one thing at a time. Structured concurrency (nurseries) appears only when "I want two slow things at once" first arises, which is genuinely late — and `program` inputs (§11) defer most in-program I/O, so even the first `await` lands well after the early lessons. Scheduling is at the **VM level** (suspension points are natural fuel-yield points, §12).
...
- **Absence is `Optional<T>`, spelled `T?`** (§4). **Failure-with-a-reason is `Result<T, E>`** — a two-case union `Success{ value: T } | Failure{ error: E }` — with the surface spelling **`T orfail E`** (`fix parse = fn(s: String): Int orfail ParseError`). Both `T?` and `T orfail E` are sugar for one underlying union; `Result<T, E>` stays writable for generic code and aliases (`type IOResult<T> = Result<T, IOError>`). `orfail` reads "a `T`, or a failure `E`" — its `fail` threading straight into the `Failure` constructor you get on the bad case — a *returned value*, never a thrown, stack-unwinding exception.
- **`match` is the full handler.** A `Result`/`Optional` is just a union, so you open it with the exhaustive `match` you already have, both cases handled, the `Failure`'s reason in hand. No new construct.
- **`try` is the propagation shorthand**, spanning both `Optional` and `Result`: `try expr` unwraps the good case and continues, or **early-returns the bad case from the enclosing function**. It desugars to exactly that match — `fix lines = try readLines(path);` ≡ `match readLines(path) { Failure{ error } -> return Failure{ error }, Success{ value } -> value }`. Because it early-returns the bad case, **a function that uses `try` must itself return a compatible `Optional`/`Result`** — the compiler enforces it, so fallibility is forced into the signature and cannot hide. Every propagation point is *visible* (you see each `try`) and *typed* (the enclosing function admits it can fail) — the exact opposite of exceptions.
- **`??` is the gentle Optional default — Optional only.** `opt ?? fallback` takes the value or, on `None`, the default. It is *not* allowed on `Result`: a `None` carries no information so defaulting it discards nothing, but a `Result`'s `Failure` carries a reason, and silently dropping that reason is exactly the dishonesty Ascent refuses. So seeing `??` tells you the left side is an Optional. `Result` errors must be *acknowledged* — handled (`match` / `try` / `try…else`) or surfaced (`.orAbort()`, below) — never silently defaulted away.
- **`.orAbort(message?)` is the escape hatch that *reports the error*.** A method on `Result`/`Optional`: it unwraps the good case, or aborts through the bug-tier crash (§9 format: location + locals). On a `Result` the abort **reports the carried `Failure`** — the most informative thing available — so the default for "I don't want to handle this" surfaces the real reason instead of throwing it away. The optional message *augments*, never replaces: `config.orAbort("loading settings")` shows your context **and** the underlying error. On an `Optional` (no error to carry) it aborts with a locator, plus the message if given — the honest "I asserted this is present." It is a *method* precisely because the receiver is the value, so it can read the `Failure` that a bare keyword cannot. This is the "abort and log the error" default; reach for it in scripts and proven-safe spots, and — being a visible call — every such gamble is greppable.
...
- **`try expr else e -> mapExpr` maps the error before propagating.** On the bad case it binds the error to `e`, evaluates `mapExpr` to a *new* error, and early-returns that — adapting a foreign error into the function's declared type, explicitly, at the call site:
  ...
  fix solve = fn(path: String): Int orfail SolveError {
      ...
      Success{ value: sum(nums) }
  }
  ```
  It adds no new concept — it is the desugared `Failure` arm made visible (`Failure{ error: e } -> return Failure{ error: mapExpr }`), reusing `match`'s `->` "arm produces" shape. So **`try` is the no-mapping shorthand and `try … else` the mapping form.** (For an `Optional`, whose bad case carries nothing, the binding is dropped: `try opt else -> SomeError{...}` turns a `None` into a propagated error.)
...
**A minimal prelude — just `print` — is ambient; every other function is imported.** Almost nothing is built in: no global `len`, no standard library dumped into scope — *every function but one is a library capability reached through an explicit import*, so provenance stays local. The single exception is **`print`**, ambient (usable with no import) because it is genuinely useful in the window *after* a learner wants simple line output but *before* the module system is taught. `print` takes a **`String`** and emits it followed by a newline; stringification goes through `${}` interpolation (the one mechanism, §4), so `print("count: ${n}")` is the idiom and `print` stays a dead-simple emit-a-line primitive — the rare no-newline case is a later `printInline`. This is a **closed, non-shadowable prelude of exactly one**, the same category as the built-in constructors (§2), not an open global soup: enlarging the prelude is a deliberate language decision, never a slope. This still costs a beginner almost nothing elsewhere, because three earlier choices keep *most* functions late: output can also be the program's returned value (the block-value rule, §2); input is a `program`'s parameters (§11); and inspecting a value mid-development is a *tooling* concern (the REPL echo / playground inspector, §13). So beyond `print`, a learner computes with values, `program` inputs, `match`, methods, and their own definitions — and the first *imported* function arrives only when a genuine library capability is needed, by which point the module system is no longer daunting. One distinction stays crisp: the language's built-in **vocabulary** — its types (`Int`, `String`), constructors (`True`, `None`, `Success`), and operators (`div`, `mod`, `**`) — is *not* a function prelude; it is the language itself, ambient like grammar. "Everything is imported" governs *functions* (with the single, deliberate `print` exception), never the built-in vocabulary — you no more import `Int` than you import `+`.
...
**The program itself — `program (params) { body }`, or a bare sequence of statements.** The simplest Ascent program is just a sequence of statements; the block-value rule (§2) makes its last value the output, so the first program is literally:

```ascent
"Hello, world!"
```

No ceremony, no entry-point keyword. When a program needs **named inputs**, it wraps its executable part in an explicit **`program (params) { body }`** — the entry point spelled as a function: a parameter list (the inputs) and a body block.
...
- **`program` *is* `main`, made literal — which dissolves the ordering question.** A file is imports (§10), then top-level **declarations** (types and `fix name = fn(...)` helpers), then the program's executable part. Earlier this project treated the input clause as a rigid middle section between imports and body; `program (params) { body }` makes it instead an ordinary *declaration with a visible body block* — so helper functions and types simply sit **above** `program` (you define `parseLine` before the entry point that uses it — the natural order), and the body is delimited by braces rather than by a rule about what may follow the inputs. The structure now lives *in the syntax*: everything inside `program { … }` runs; everything outside it defines. Imports still group contiguously at the top (§10); `program` is an ordinary declaration, conventionally last.
- **`program` is optional.** With no named inputs you skip it — a bare sequence of top-level statements is a complete program (the block-value rule gives its output). `program (params) { … }` is what you reach for once you take inputs or want the explicit entry-point structure, so the trivial first program stays trivial and the structure appears exactly when it earns its place. Each parameter is an ordinary fixed slot whose initializer is the *user* rather than a literal — not a new slot kind; the required annotation is honest, since there is nothing to infer from.
- **No declared output type.** `program` does not annotate a return type; its value is its body's last expression (block-value rule, §2), which the environment displays or uses. (Exiting a program *early* is a separate concern — it applies to any Ascent program, not just `program` — and is deferred.)
- **`program` is terminal — nothing may follow it.** When the explicit `program (params) { body }` form is used, it is the **last** thing in the file. Everything *before* it is unrestricted: imports (§10), `type` declarations, function bindings, **and** input-independent executable statements (setup that does not need the input — `fix config = loadDefaults()` and the like) run top-to-bottom on the way down. This is exactly what `program` is *for* — it walls off the **input-dependent** part of the program as a clearly-marked finale, leaving whatever doesn't depend on the input above it. Code *after* `program` is an **error**: `program`'s body is that finale, and its block value is the program's output (block-value rule, §2), so there is no meaningful position after it — anything there either cannot run (the output is already produced) or would be a second, competing program. The sequence is always: setup runs, inputs are gathered and validated, the body runs, the body's last value is the result.
- **Empty parentheses are an error — write `program { … }`, not `program () { … }`.** The parentheses hold the parameter list, so they appear *only* when there is one; with no inputs you write `program { body }`. Unlike a function — whose `fn()` parens mirror the `f()` call site — `program` is never called in source (the environment invokes it), so its parentheses are pure declaration and empty ones are noise. `program () { … }` is rejected with a fix-it (drop the `()`), keeping a single spelling for the no-input entry point.
- **Inputs are gathered and validated at the boundary before the body runs.** The environment reads `program`'s parameter list, builds a fitting input dialog (one field per parameter), collects the values, and **validates each to its declared type** — type "abc" into an `Int` field and it re-asks, so the body never runs with a bad value (§6: external data is parsed into a declared type at the boundary). By the time the first body line executes, every parameter already holds a valid value, so the body stays synchronous and pure — no `await`, no effects.
- **What makes a type a valid `program` parameter:** it must have a single canonical input widget *and* a total-or-cleanly-validating parse from what the user types — because the parameter list is a boundary, every admitted type is a widget to render and a garbage-input failure to handle gracefully. **v1 allows the four scalars**, each with one obvious control: `String` → text field (any text is valid, so it never fails), `Int` / `Float` → number field (re-asks on `"abc"`), `Bool` → checkbox. (The CLI supplies the same values as flags / stdin instead of a modal.)
...
- **Structured values stay out of parameters** — records, field-bearing unions, `Map`, `Ref`, and function types have no honest single widget. Don't ask for a `Point`; write `program (x: Int, y: Int) { … }` and let the body build it. The boundary takes flat, named scalars and the program assembles richer values — keeping the parameter list a *parameter list*, not an arbitrary-data deserializer.
- **`program` over `main`, and the graduation it sets up.** The keyword says what it is; `main` is jargon (main *what*?). And because `program (params) { body }` is *exactly* a function shape — a signature and a body — a learner who later writes `fix f = fn(params) { body }` already knows the form; the entry point was their first function all along. Graduation note: other languages call this `main`, and real-world arguments (`argv`, `sys.argv`, `String[] args`) arrive as a raw positional list of *strings* the program parses itself — Ascent names them and checks their types, the same idea with training wheels, removed later by a `prompt()`-style raw-`String` input once functions exist.
...
- **MVU, pure.** A panel is three pure pieces: a `Model`, `view : Fn(Model) -> Element`, and `update : Fn(Model, Message) -> (Model, Command)`. `view` *returns a description* of the screen (it never draws); `update` *returns* the next model and a *description* of any effect (it never performs one). Both stay ordinary pure functions.
...
- **Failure re-enters as a message.** A command that can fail (`httpGet` is `... orfail NetError`) is run by the runtime; its `Result` returns as a `Loaded{...}` or `Failed{...}` *message*, handled by pure `update` via `match`. Errors flow through the §9 model as ordinary data — never exceptions, never unhandled rejections.
- **Subscriptions** are the same idea for *ongoing* effects: a pure `Fn(Model) -> Subscription` value declaring "while in this state, listen to this timer / socket." The runtime **diffs** it as the model changes and runs the active ones as model-scoped structured-concurrency tasks (§8), so starting, stopping, and resource cleanup fall out of nursery-scope cancellation automatically. This is where the old `with` / resource-cleanup question resolves.
- **One boundary, three positions.** A `program`'s inputs (*before* the run), `await` (waiting *during* it), and commands (effects out, results back as messages) are the same edge — between pure computation and the uncertain outside world — at different times. A teaching spine, not three disconnected features.
...
type Shape =
    ...
    | Rect{ width: Float, height: Float }
methods {
    area: fn(self): Float { match self {     # a method, block body
        Circle{ radius }      -> 3.14159 * radius * radius,
        Rect{ width, height } -> width * height,
    } },
};
