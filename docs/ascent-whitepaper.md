...
- **With an explicit `program`, executable code lives *only* inside it.** When the `program (params) { body }` form is used, the file's top level may hold only imports (§10) and declarations — `type`s and `fix name = fn(...)` function bindings — plus the single `program` block. A bare **executable statement** outside `program` (a call, an `await`, a computed value binding) is an **error**: a bare statement sequence is *itself* a program (the implicit form above), so a loose statement beside an explicit `program` would be a *second, competing* program, and "which one runs?" has no honest answer. You write one or the other — a bare statement sequence, *or* an explicit `program` with all execution inside its braces — never both. Declarations are not execution, so types and helper functions above `program` are fine; that is the normal shape.
