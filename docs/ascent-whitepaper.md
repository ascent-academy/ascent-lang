...
- **`match subject { }`** — an expression, **exhaustiveness-checked**. v1 patterns are **shallow**, exactly three kinds: **literal** (`0`, `"hi"`, `True`), **variant** (`Circle{ r }` — matches a union case and binds its fields), and the **`else` catch-all** (the default arm — reusing the `else` you already know from `if` and `try … else`, so "0, 1, or **else** many" reads with no explanation, and there is no cryptic `_`). A variant pattern may bind a **subset** of the variant's fields — the ones you name are bound, the rest are simply ignored (`Circle{ r }` binds `r`, ignores any other field) — so there is no separate field-wildcard to learn either. (No nested patterns, guards, or or-patterns in v1.) Chosen over `switch` to avoid fallthrough/`break` expectations. **Exhaustiveness + `else`:** a `match` must cover every case — list all variants, *or* supply an `else`. An `else` makes a `match` trivially exhaustive, which is convenient but **masks future variants**: add a `Blue` to a `Color` you matched with `Red` / `Green` / `else`, and the `else` silently swallows it instead of the checker flagging the gap — so prefer listing variants explicitly when you want the compiler to catch later additions, and use `else` only when the remaining cases genuinely do not matter.
