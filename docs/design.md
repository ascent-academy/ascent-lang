...
- **Identifiers**: `[A-Za-z_][A-Za-z0-9_]*`. Keywords (`fix`, `mut`, `and`, `or`, `not`, `div`, `mod`, `true`, `false`, `args`, `try`, `orelse`, `abort`, `import`, `export`, `from`, `async`, `await`, `nursery`, `trait`, `implement`, `requires`, `Self`, and the control-flow/type words) are reserved. The last four are held ahead of the traits feature (§16) — reserved now so no program breaks when it lands, even though they are not yet usable.
- **Naming & casing.** Lowercase (`lowerCamel`) for **bindings** — variables, functions, fields, parameters — and for the language's **built-in literals** (`true`, `false`, `none`, numbers, strings). Uppercase (`UpperCamel`) for every name a `type` declaration introduces — **both the type and its variant constructors**: `type Color = Red | Green | Blue` makes `Color`, `Red`, `Green`, `Blue` all uppercase, because the constructors are values *born from* the type. So the rule is "names you declare via `type` are uppercase; ordinary bindings and built-in literals are lowercase," not "uppercase = type." The type/constructor overlap is harmless because they never share a syntactic slot — a type appears only after `:` or `->`, a constructor only in value and pattern positions — so `Color` (the type) and `Red` (a value) are never mistakable in context, as decades of Haskell and Elm confirm. Built-in literals stay lowercase precisely because they are primitive values like `0` or `""`, not constructors you could have declared.
...
- **Operator precedence**, loosest to tightest: `or` · `and` · `not` · comparisons (`== != < <= > >=`, non-associative — no chaining) · `+ -` · `* / div mod` · unary `-` · atoms (literals, identifiers, parenthesized expressions). Binary arithmetic is left-associative. Follows Python in one respect: `not` binds looser than comparison, so `not a == b` parses as `not (a == b)`. The expression parser is Pratt-style (§12).
...
- **Receiver mutability — `self` reads, `mut self` mutates.** By default a method's `self` is **read-only** (it observes the receiver); marking it **`mut self`** declares that the method *modifies the receiver in place*. Reading is the unmarked norm because in a value-semantics language (§3) the vast majority of methods return new values — so mutation is the rare, *always-marked* exception, and nothing is hidden (a mutating method always says `mut self`) while reading methods stay noise-free. This is a deliberate, reasoned departure from the "state `fix`/`mut` on every slot" rule (§3): for value *slots* both choices are common so both are stated, but for a *receiver* reading so dominates that marking only the mutating exception is the honest, low-noise form. `sorted: fn(self) -> List<Int>` returns a new list; `sort: fn(mut self) -> Done` reorders in place. The compiler enforces the mark in **both directions**: at the *definition*, a plain-`self` method that tries to mutate the receiver is rejected (so `mut self` can't be silently omitted where it is needed); at the *call site*, a `mut self` method is callable **only on a `mut` binding**, while a plain-`self` method works on **anything** — a `mut` slot, a `fix` slot, or a bare literal (reading is always allowed). Mutation is therefore a fact of the *signature*, judged from the interface and never traced through the body — the black-box rule (§10) holds for mutation too. This is what makes the `sort`/`sorted` naming *enforced* rather than hoped-for, and it settles the boundary cases by one rule: `mut xs; xs.sort()` is fine, but `fix xs; xs.sort()` and `[1, 2, 3].sort()` (a literal is a temporary, hence read-only) are both compile errors that name the fix — use a `mut` binding, or the returning variant `sorted`. (Receiver mutability is thus part of a method's *type*, so when traits arrive (§16) a trait method fixes `self` vs `mut self` and implementors must match — Swift's `mutating`, made honest.)
- **Inherent immutability falls out — no separate concept needed.** A type is *inherently immutable* — no instance can ever change, in any slot — exactly when it declares **no `mut self` methods**. That is how the built-in value types already work: `Int`, `String`, `Bool` have only reading methods, so a `mut n = 5` slot can be *reassigned* but the value `5` can never mutate. To make your own value-object — a `Money`, a `Color`, a coordinate — give it only reading methods and it is immutable everywhere, automatically. A dedicated *frozen* / *deeply-immutable* keyword (which reference-semantics languages need) is **not** added: value semantics (§3) already removes the danger it guards against — no value is ever shared by reference, so nothing can be mutated through an alias — so immutability here is simply the *absence of mutating methods*, not a defensive annotation.
...
- **Collection mutation & indexing — mostly settled; two specifics open.** Decided: mutation is governed by `mut self` vs plain `self` (§6) — `sort` needs a `mut` binding, `sorted` returns a new value; and indexing has two accessors (§9) — `xs[i]` yields `T` and crashes out-of-bounds (bug tier), `xs.at(i)` yields `T?` for guarded access. Still open: element assignment (`xs[i] = v` on a `mut` binding — allowed, and does it crash out-of-bounds like read indexing?), and the names of the *returning* partners of the grow/shrink family (`append` → `appended` / `plus` / `with`, etc.).
