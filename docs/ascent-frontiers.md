...
**No tuples — anonymous positional products are out.** A tuple `(Int, String)` is an *anonymous* product accessed *by position* (`.0` / `.1`), and both properties are things Ascent excludes on principle: anonymous aggregates are out (§6 — construction needs a declared type), and positional access hides meaning in an index rather than a name (a returned index-accessed tuple is terrible design — the thing to avoid). Every real tuple use is better served by a **named** type: multi-value return is a declared record (`DivResult{ quotient, remainder }` — self-documenting), map iteration yields a named `Entry{ key, value }`, and `zip`-style pairing uses a stdlib `Pair<A, B>{ first, second }`. The one genuinely good thing tuples give — **one-line destructuring** — Ascent keeps *without* them, via irrefutable patterns in a `fix` / `mut` binding (`fix DivResult{ quotient, remainder } = divide(a, b)`, §6): the good half (multi-value binding) on named, order-independent, typo-checked fields, minus the bad half (positional `.0` / `.1`).

**Deferred** — a "later module," introduced when a learner asks the question it answers: interfaces / typeclasses (traits) · user-definable generics · exceptions · operator overloading · default / named arguments · placeholder sections (`T{ field: _ }` as a function, with partial application) · varargs · comprehensions · getters / setters · decorators · macros · `Set` · `Char`.
