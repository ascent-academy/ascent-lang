...
describe('void and the discard rule', () => {
  describe('T0057 — a value dropped by a following statement', () => {
    it('flags a non-final bare value at the top level', async () => {
      assert.deepEqual(errorCodes('1 + 1; 2;'), ['T0057']);
      assert.deepEqual(errorCodes('"a"; "b";'), ['T0057']);
    });

    it('flags a non-final bare value inside a block (value-position branch)', async () => {
    ...
    it('does not flag a non-final statement that already yields Done', async () => {
    ...
    it("does not flag the program's final statement (a value position)", async () => {
      assert.deepEqual(await evalOk('1 + 1;'), { type: 'Int', value: 2n });
    });

    it("'void' discards the value and clears the error", async () => {
      assert.deepEqual(errorCodes('void 1 + 1; 2;'), []);
      assert.deepEqual(await evalOk('void 1 + 1; 2;'), { type: 'Int', value: 2n });
    });

    it('a statement-position if used for effect takes void on the whole expression', async () => {
  ...
  describe('T0058 — a value dropped by the loop', () => {
    ...
    it('a loop still accumulates through a mutable slot', async () => {
      assert.deepEqual(await evalOk('mut s = 0; for i in 0..3 { s = s + i }; s;'), { type: 'Int', value: 3n });
    });
  });

  describe('T0059 — void on a value that is already Done', () => {
    it("flags 'void' on a Done expression", async () => {
  ...
  describe('void semantics', () => {
    it('a void statement yields Done', async () => {
      assert.deepEqual(await evalOk('void 2 + 2;'), { type: 'Done' });
    });
