...
describe('void and the discard rule', () => {
  describe('T0025 — a value dropped by a following statement', () => {
    it('flags a non-final bare value at the top level', () => {
      assert.deepEqual(errorCodes('1 + 1; 2;'), ['T0025']);
      assert.deepEqual(errorCodes('"a"; "b";'), ['T0025']);
    });
    ...
    it('flags a non-final bare value inside a block (value-position branch)', () => {
      ...
      assert.deepEqual(errorCodes('fix x = if (True) { 7; 8 } else { 9 }; x;'), ['T0025']);
    });
    ...
    it('a statement-position if used for effect takes void on the whole expression', () => {
      assert.deepEqual(errorCodes('if (True) { 5 } else { 6 }; 0;'), ['T0025']);
      assert.deepEqual(errorCodes('void if (True) { 5 } else { 6 }; 0;'), []);
    });
  });

  describe('T0026 — a value dropped by the loop', () => {
    it('flags a for/while body ending in a non-Done value', () => {
      assert.deepEqual(errorCodes('for i in 0..3 { i };'), ['T0026']);
      assert.deepEqual(errorCodes('while (False) { 1 };'), ['T0026']);
    });
    ...
    it('a non-final drop and a loop drop can both fire in one body', () => {
      // First 'i' is dropped by the statement after it (T0025); the last 'i' is
      // dropped by the loop (T0026).
      assert.deepEqual(errorCodes('for i in 0..3 { i; i };'), ['T0025', 'T0026']);
    });
  ...
  describe('T0027 — void on a value that is already Done', () => {
    it("flags 'void' on a Done expression", () => {
      assert.deepEqual(errorCodes('void Done;'), ['T0027']);
      assert.deepEqual(errorCodes('void print("x");'), ['T0027']);
    });
  });
