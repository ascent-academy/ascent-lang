...
function evalCrash(src: string): Marker {
  const { program, diagnostics } = parse(src);
  assert.deepEqual(diagnostics, [], `unexpected errors: ${diagnostics.map(d => d.code).join(', ')}`);
  assert.ok(program !== null, 'expected the program to typecheck');
  const result = executeProgram(program, { stdout: () => {} });
...
describe('abort (end-to-end)', () => {
  describe('as a diverging expression (type Never)', () => {
    it("satisfies its neighbour's type in a match arm", () => {
  ...
  describe('runtime crash (R0008)', () => {
    ...
    it("the rendered message carries the reason, not the source '{found}'", () => {
...
describe('.orAbort() on Result / Optional (end-to-end)', () => {
  ...
  describe('Result', () => {
    it('unwraps a Success to its value', () => {
      assert.deepEqual(evalOk(`${parseFn}parse(True).orAbort();`), { type: 'Int', value: 42n });
    });

    it('the unwrapped value has the ok type (usable in arithmetic)', () => {
