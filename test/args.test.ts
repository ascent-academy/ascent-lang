...
async function evalWithArgs(src: string, args: Record<string, ScalarValue>): Promise<RuntimeValue> {
  ...
  const result = await executeProgram(program, testHost(), inputs);
...
describe('program inputs (program (…) { … } header)', () => {
  describe('declaration & binding', () => {
    it('exposes a declared input as a readable name', async () => {
      assert.deepEqual(
        await evalWithArgs('program (age: Int) { age }', { age: { type: 'Int', value: 21n } }),
        { type: 'Int', value: 21n },
      );
    });

    it('binds several inputs at once, each by name', async () => {
      assert.deepEqual(
        await evalWithArgs('program (a: Int, b: Int) { a + b }', {
          a: { type: 'Int', value: 2n },
          b: { type: 'Int', value: 3n },
        }),
        { type: 'Int', value: 5n },
    ...
    it("threads an input's declared type through the body", async () => {
      // 'flag' is a Bool, so it drives an 'if' without any coercion.
      assert.deepEqual(
        await evalWithArgs('program (flag: Bool) { if (flag) { 1 } else { 2 } }', { flag: { type: 'Bool', value: true } }),
        { type: 'Int', value: 1n },
      );
    });

    it('accepts a String input and runs its methods', async () => {
      assert.deepEqual(
        await evalWithArgs('program (name: String) { name.length() }', { name: { type: 'String', value: 'hi' } }),
        { type: 'Int', value: 2n },
      );
    });

    it('accepts an Int input at the exact 64-bit boundaries', async () => {
      const INT_MAX = 2n ** 63n - 1n;
      const INT_MIN = -(2n ** 63n);
      assert.deepEqual(
        await evalWithArgs('program (n: Int) { n }', { n: { type: 'Int', value: INT_MAX } }),
        { type: 'Int', value: INT_MAX },
      );
      assert.deepEqual(
        await evalWithArgs('program (n: Int) { n }', { n: { type: 'Int', value: INT_MIN } }),
        { type: 'Int', value: INT_MIN },
      );
    });

    it('rejects an Int input outside the 64-bit range at the boundary', async () => {
      ...
      // evalWithArgs is async, so the rejection is a rejected Promise, not a
      // synchronous throw — assert.rejects, not assert.throws.
  ...
  describe('the read-only rule', () => {
    it('reports N0004 for assigning to a program input', async () => {
    ...
    it('still type-checks the assigned value alongside N0004', async () => {
  ...
  describe('inputs participate in the type system', () => {
    it('rejects a body that misuses an input\'s type', async () => {
  ...
  describe('the header is optional', () => {
    ...
    it('checks the empty inputs first when both parens and body are empty', async () => {
  ...
  describe('statements before the program block', () => {
    it('runs type and value declarations before program, visible in the body', async () => {
      assert.deepEqual(
        await evalWithArgs('type Point = { x: Int, y: Int }; fix origin = Point{ x: 1, y: 2 }; program (n: Int) { origin.x + n }', { n: { type: 'Int', value: 10n } }),
        { type: 'Int', value: 11n },
      );
    });

    it('defines a helper function before program and calls it in the body', async () => {
      assert.deepEqual(
        await evalWithArgs('fix dbl = fn(x: Int): Int { x * 2 }; program (n: Int) { dbl(n) }', { n: { type: 'Int', value: 7n } }),
        { type: 'Int', value: 14n },
      );
    });
    ...
    it('runs an effectful statement before program, then the body', async () => {
      ...
      await executeProgram(program, testHost(t => output.push(t)), inputs);
      assert.deepEqual(output, ['setup', '3']);
    });

    it('a bare non-Done value before program is still a dropped value (T0057)', async () => {
      assert.deepEqual(errorCodes('fix x = 5; x + 1; program (n: Int) { n }'), ['T0057']);
    });

    it('the inputs are NOT visible to statements before program (N0001)', async () => {
    ...
    it('rejects an empty program body, even with leading statements (S0029)', async () => {
      assert.deepEqual(errorCodes('print("setup"); program (n: Int) { }'), ['S0029']);
    });

    it('still requires a semicolon before program, like any statement (S0006)', async () => {
      assert.deepEqual(errorCodes('fix a = 1 program (n: Int) { n }'), ['S0006']);
    });

    it('rejects anything after program even with a separating semicolon (S0030)', async () => {
